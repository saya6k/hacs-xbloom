...
- The 8002 echo ACK does not mean the brew started — `brewing._async_verify_brew_started` watches the raw state heartbeat (`starting`/`brewing` = started; `water_shortage`/`no_beans` = refused; `awaiting_confirm` = the machine wants a human at its screen) and raises `BrewStartUnconfirmed` otherwise. It sends **nothing** while watching: 40518-as-start is refuted on this machine (twice now — live 2026-07-19 it bounced `awaiting_confirm` back to `recipe_loaded`) → [[xbloom-brew-start-verification]]
- Recipe cancel is **bare 40519** (the app's `AppJ15AutoManager.stop()` sends only that); the old 3505/4507/8022 chasers are gone. Pause/resume's no-`_active_operation` fallback only fires 40518/40524 when state is verifiably pausable (`starting`/`brewing`/`grinding`/`paused`), else no-op — 40518 from the wrong state is start-or-abort roulette → [[xbloom-brew-start-verification]]
