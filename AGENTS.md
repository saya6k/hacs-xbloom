...
- **The standalone manual Grinder/Brewer screens (8018/8019/8020/8021 pause/restart, `APP_GRINDER_STOP`/`APP_BREWER_STOP`, entered via `APP_GRINDER_IN`/`RD_BREWER_IN`) are decompile-documented but deliberately not implemented — no concrete use case, same reasoning as the reverted cloud Product/Shared recipe tabs.** This is a real, independent app feature ("just grind beans" / "just run water" without a recipe — e.g. `GrinderActivity`'s manual grind-size/RPM adjustment via cmd `3500`, `BrewerActivity`'s manual pour), not a duplicate of anything else in this integration. If a future need for standalone manual grind/pour control surfaces, the commands are already identified above — implement then, not speculatively.
...
- **A command-table completeness sweep (2026-07-17) turned up 8 unhandled command IDs. An initial pass (androguard ID-sweep + command names only) implemented 2 and guessed at the rest; a follow-up `jadx` decompile of `xbloom_coffee_release.apk`'s actual model classes (`com/chisalsoft/andite/model/ble/*BleModel.java`) corrected two of those guesses and turned 3 more "no concrete use case" items into real, payload-confirmed features.** Lesson reinforced: `androguard`'s ID sweep tells you a command exists; only a `jadx`-level source read of the class `BaseBleModel.Companion.create()` dispatches to (per command id) tells you what the payload means and where it's actually triggered from — don't guess from the command name.
  ...
  - **Grinder gear-position calibration — cmd `3502` trigger + `50038`/`50039`/`40526==85` progress/completion, implemented 2026-07-17 as a bundle.** Previously documented (see the "Advanced Features" bullet above) as a known command with no HA-side trigger. `jadx`: `CalibrateGrinderActivity`'s confirm button sends `CodeModule(3502, "磨豆档位归0", 1000)`; `CalibrateStartModel`/`CalibratingModel` (50038/50039) are bare start/in-progress pulses (no payload — confirmed, not just assumed); completion is `CurrentGrinderBleModel.value == 85` observed while a calibration is in flight (`CalibrateGrinderActivity` checks this exact equality). `brewing.async_calibrate_grinder()` sends 3502; `_client.py` tracks `self._status.is_calibrating_grinder` (set on 50038, gates the `raw == 85` check on 40526, cleared on completion) and fires `grinder_calibration_started`/`_progress`/`_complete`. New `button.calibrate_grinder` (grinder sub-device). **50038/50039's names ("Calibrate…") originally led this integration to (wrongly) suspect they were the unresolved *scale*-calibration trigger** — they're grinder-calibration, a different, already-partly-documented feature; the scale-calibration trigger is still unresolved, see the "Advanced Features" bullet.
  - **`8017` (`APP_RECIPE_START_QUIT`/quitRecipeStart) → `button.dismiss_pod`, implemented 2026-07-17 — corrects an earlier wrong dismissal.** The initial pass concluded this had "no applicable scenario in our architecture" because our own brew flow sends `8001/8004 → 8002` back-to-back with no confirmation gap. That reasoning doesn't apply here: `jadx` call-site tracing shows `quitRecipeStart()` is sent from `PodsDetailActivity`/`RecipeDetailActivity`'s `showStartDialog()` dismiss handler, **before any BLE brew command is sent** — the machine shows its own local "ready to brew this pod" prompt the instant it reads the NFC tag (the same `RD_Pods`/40501 event now exposed as `pod_detected`), independent of whether the app/HA has armed anything. `brewing.async_dismiss_pod_prompt()` sends bare 8017; pairs naturally with `pod_detected` in an automation ("pod scanned, but I don't want to brew it — dismiss").
