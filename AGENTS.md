...
- **`sensor.state` can get permanently stuck at `unknown` on a connected machine that simply hasn't brewed yet this session — fixed 2026-07-17.** Hardware-reported: state stayed "알 수 없음" indefinitely despite a healthy connection. Root cause: the vendored `DeviceState` enum (`src/xbloom/models/types.py`) defaults to `UNKNOWN` and is only ever transitioned to `IDLE` by a `RD_Grinder_Stop`/`RD_Brewer_Stop` event (`src/xbloom/core/client.py:297-307`) — i.e. only *after* at least one grind/brew cycle has completed since connecting. `_RAW_STATE_LABEL_MAP` (the override layer described above) has no idle code either — it only maps `starting`/`brewing`/`ready`. Net effect: a machine that's connected and genuinely idle, but hasn't ground or brewed anything yet this session, has no code path that ever sets its state away from the enum default, so it reports `unknown` forever instead of `idle`. `coordinator._async_update_data` now maps `s.state.value == "unknown"` to `"idle"` when we're inside the `client.is_connected` branch (i.e. connected, no error/no_beans/water_shortage, no raw_label match) — connected + no activity ever observed is, definitionally, idle. Doesn't touch the vendored enum/file.
...
- **The above only "mirrored exactly" the ACK-timeout part — the `isSleeping()` gating condition itself was unimplemented until 2026-07-17, since this integration never tracked machine sleep state at all.** Found via a systematic decompile-vs-integration gap sweep of `xbloom_coffee_release.apk`: the real app's retry loop only retries `while (AppDeviceManager.INSTANCE.isSleeping())` — a missed ACK while the machine is *awake* fails immediately after one send, no retry at all — but our port retried unconditionally up to 4 times regardless of sleep state. Root cause was one level deeper: cmd `8009`/`8011`/`8023` (`RD_MachineSleeping`/`RD_MachineNotSleeping`/`RD_MachineActivity`) are already valid `XBloomResponse` enum members (`src/xbloom/protocol/constants.py`) but had no handler in `_handle_response` at all, so nothing ever set a sleep flag to gate on. Decompiled from `MachineSleepingModel`/`MachineNotSleepingModel`/`MachineActivityModel.excute()` (`com/chisalsoft/andite/model/ble/`, via `jadx`) — 8009 sets sleeping, 8011 and 8023 both clear it (8023 also carries a payload `index` field the app only uses to conditionally post an RxBus UI event; `excute()` clears the flag unconditionally regardless of `index`, so we don't parse it). `_client.py`'s `_handle_response` now sets `self._status.is_sleeping` on all three (ad-hoc dataclass attribute, same pattern as `_mode_bytes`/`_raw_state_label`/`_mode_ack_hex` — `DeviceStatus` isn't `@dataclass(slots=True)`), exposed via `client.is_sleeping()`; `_async_switch_mode_with_retry()` now `break`s out of the retry loop on the first ACK timeout while `not client.is_sleeping()`, instead of always exhausting all `_MODE_SWITCH_MAX_ATTEMPTS`. No raw pre-scan needed (unlike the advanced-settings commands) — these are ordinary type-1 notifications the vendored enum already resolves. Tests in `tests/test_sleep_state.py`. **Not hardware-verified** — like the rest of this gap sweep, found and implemented from the decompile alone; the practical effect only shows up on a real machine that's actually gone idle-to-sleep between mode switches.
- **The recipe pause/resume button sent the wrong command family entirely — `APP_GRINDER_PAUSE`/`APP_BREWER_PAUSE` (8018/8019) instead of the app's actual whole-recipe pause (40518/40524) — fixed 2026-07-17.** Traced by decompiling every call site of `CommandParams.APP_GRINDER_PAUSE`/`APP_BREWER_PAUSE`/`APP_GRINDER_RESTART`/`APP_BREWER_RESTART` (`xbloom/others/CommandParams.java`'s official names, matching our vendored PyBloom's `grinder.pause()`/`brewer.pause()` naming exactly — which is what made this look correct for years): every real usage is in `GrinderActivity`/`BrewerActivity` (`com/chisalsoft/andite/uicontroller/activity/`), two **standalone manual single-motor screens** reached only from the home screen's own "Grind"/"Brew" quick-action icons (`HomeFragment.doGrindJ15()`/`doBrewer()`), which first send a distinct "enter mode" handshake (`APP_GRINDER_IN`/`RD_BREWER_IN`, "进入磨豆页面"/"进入浇水页面" = "enter grind/pour page") before the pause-capable screen even opens. This is a different machine mode from the onboard Auto-recipe state our own `8001`/`8004`+`8002`-driven brews put the machine into — which is exactly what `AppJ15AutoManager.pause()`/`restart()` (40518/40524, "全流程冲泡暂停"/"暂停之后开始" = "full-flow brew pause"/"start after pause") target, and the *only* pause mechanism the app's own in-recipe pause button ever calls. `coordinator.async_pause_resume()` now sends `_CMD_RECIPE_PAUSE`(40518)/`_CMD_RECIPE_RESTART`(40524) — bare commands, no payload, matching the app's own `CodeModule(40518, ...)`/`CodeModule(40524, ...)` calls. This also reframes the earlier "40518 resets to armed" hardware finding (see the `RD_MachineInfo`/systemStatus bullets above): that test never sent 40524 afterward, so the "stuck at armed" outcome may simply be what an incomplete pause→(no resume) sequence looks like, not evidence 40518 itself misbehaves — genuinely unresolved until a real pause→40524-resume round trip is captured. **Not hardware-verified** — this swap has not been tested on a real machine yet; if a paused recipe fails to resume via 40524, capture the actual BLE traffic (does the machine echo anything, does state ever leave "paused"?) before reverting to 8018/8019.
- **The standalone manual Grinder/Brewer pause/restart/stop (8018/8019/8020/8021, `APP_GRINDER_STOP`/`APP_BREWER_STOP`) are wired up as of 2026-07-17 — this reverses the earlier "no concrete use case, don't implement" call below (kept for history).** `button.grind`/`button.pour` (`coordinator.async_grind()`/`async_pour()`) already existed and already worked — `GrinderController.start()` (vendored, `src/xbloom/components/grinder.py`) already sends `APP_GRINDER_IN` (8006) internally before `APP_GRINDER_START` (3500); that part was never missing. What was missing: `button.pause`/`button.cancel` always sent the whole-recipe commands (40518/40524 pause/restart, 40519+stop+8022 cancel) regardless of what was actually running, which is wrong for a manual grind/pour — the vendored `GrinderController`/`BrewerController` already had their own `pause()`/`restart()`/`stop()` (8018/8020/3505 grinder, 8019/8021/4507 brewer) but nothing called them. Fixed by adding `coordinator._active_operation` (`"recipe"`/`"manual_grind"`/`"manual_pour"`/`None`, set in `async_grind()`/`async_pour()`/`async_execute_recipe()`, cleared in `_dispatch_event()` on the matching completion event — `grinding_complete` only clears it when the value is specifically `"manual_grind"`, since a coffee recipe's own grind phase fires the same event mid-recipe) — `async_pause_resume()`/`async_cancel()` now branch on it to target the right command family. Explicitly **not** new entities — extends the existing single pause/cancel buttons (user's choice, matching the `_pod_prompt_active` pattern the `dismiss_pod` merge established the same day) rather than adding dedicated manual-grind/manual-pour buttons. Also added: `coordinator.async_pour()` now sends `8007` (`RD_BREWER_IN`, `brewing._CMD_BREWER_IN`) before `client.brewer.start()`, for app parity — not functionally required, 4506 alone was already hardware-confirmed sufficient. Status monitoring through completion needed no new events — `grinding_started`/`_complete` and `brewing_started`/`pour_complete` already fire from the raw hardware notifications regardless of what triggered the grind/pour. **Not hardware-verified** — needs a real manual grind/pour, pause mid-operation (expect 8018/8019, not 40518), resume (8020/8021), and cancel (3505/4507, not the recipe stop sequence).
- *(Historical, superseded by the above)* ~~The standalone manual Grinder/Brewer screens (8018/8019/8020/8021 pause/restart, `APP_GRINDER_STOP`/`APP_BREWER_STOP`, entered via `APP_GRINDER_IN`/`RD_BREWER_IN`) are decompile-documented but deliberately not implemented — no concrete use case, same reasoning as the reverted cloud Product/Shared recipe tabs.~~
...
- **Removed `button.vibrate_scale` (2026-07-17) — the `SG_*` scale-gesture commands (2500-2505, incl. `SG_VIBRATE` 2502) are not a real Studio feature.** Hardware-reported as a no-op, and the evidence agrees: the official app never sends any `SG_*` command anywhere (they exist only in its `CommandParams` constant table), and `src/xbloom-ble`'s capture-based PROTOCOL.md has no trace of them — the vendored PyBloom `scale.vibrate()`/`move_left()`/`move_right()` methods were written from the app's constant names alone, with no capture behind them. Don't re-expose any `SG_*`-based entity without first capturing the official app actually using it.
...
- **`RD_ErrorLackOfWater` (40522) is a bidirectional tank-state notification, not a one-shot error — the payload's first 4 bytes (LE uint32) are `0` = tank empty, `1` = water restored.** Found 2026-07-17 from a real-hardware deadlock report: water shortage fired, the user refilled, and every recipe execute stayed blocked — `coordinator._water_shortage` could only be cleared by a successful-brew notification, which `async_execute_recipe`'s own low-water gate made unreachable; worse, the firmware's own "refilled" 40522 (value=1) was being counted as *another* shortage since `_client.py` fired `("error", "water_shortage")` for every 40522 regardless of payload. Decompiled ground truth: the official app's `ErrorLackOfWaterBleModel` parses `data.substring(0,8)` → `reverseHex` → int (i.e. payload[0:4] LE, the standard integer-payload shape), and `HomeActivity`'s RxBus handler calls `dismissWaterScarcityAnimation()` on `value == 1` / `showWaterScarcityAnimation()` on `value == 0` (it also skips the warning entirely when `device.waterFeed != 0` — direct feed — matching our existing `water_source == WATER_SOURCE_TANK` gate condition). Fix: `_client.py` special-cases 40522 (removed from `_ERROR_MAP`), fires `("notification", "water_refilled")` on value=1 vs `("error", "water_shortage")` on value=0, and mirrors the value onto `status.water_level_ok` — which the vendored client otherwise only sets once from the connect-time `RD_MachineInfo` snapshot (payload[33]), so on MachineInfo-capable machines the execute gate's `water_ok` also used to stay frozen at whatever the tank looked like at connect. `coordinator._dispatch_event` clears `_water_shortage` on `water_refilled`. Untested on real hardware (the value=1-on-refill push is inferred from the app's dismiss logic, not captured live) — if a refill ever fails to clear the state on a real machine, capture the actual 40522 traffic before assuming the value semantics are wrong.
- **Display units and water source are bidirectional: cmd `8015` (RD_UNIT_CHANGE) is a machine→HA push, and water source has a real SET command (`4508`) — implemented 2026-07-17, decompile-verified, not yet hardware-verified.** From the official app's `DeviceUnitBleModel`: the 8015 payload is three LE uint32s — [0:4] weight unit (0=g/1=oz/2=ml, same codes as outbound 8005), [4:8] temp unit (0=C/1=F, same as 8010), [8:12] water source (0=tank/1=direct) — pushed when they're changed on the machine's own touchscreen (the app bails out below 12 payload bytes; so do we). `_client.py` fires it as a coordinator-internal `("settings", "unit_change")` event (the event entities filter on `error`/`notification`, so it never surfaces there); `coordinator._async_sync_units_from_machine` folds it into the stored preferences. Water source: the app's `BleCodeFactory.switchWaterFeed` sends cmd `4508` with a single LE uint32 — on Studio the value is `WaterSourceType.ordinal()` (tank=0, direct/TAP=1, matching `WATER_SOURCE_TANK`/`DIRECT`); the 8/50 values in the same decompiled helper are J20-only. Our `select.water_source` used to be HA-local (manual-pour payload + shortage gate only); `coordinator.async_set_water_source` now actually sends 4508, and `_apply_unit_preferences` re-asserts it at connect alongside 8005/8010. **The unit/water-source option keys are in `__init__.py`'s `_NO_RELOAD_OPTION_KEYS` now** — the water-source select's persist used to full-reload the entry (dropping BLE) on every change, the same latent bug shape `CONF_MODE` had; the no-reload path calls `coordinator._handle_unit_options_change`, which pushes changed values to the machine in place (and recognizes echoes of the coordinator's own persist by value equality, so 8015-sync → persist → listener can't loop).
- **Tea steeps end on `RD_TEA_RECIP_PAUSE` (40515) → "paused"** or `RD_ENJOY` (40512) → "recipe_complete", and resume on `RD_TEA_RECIP_RESTART` (9011) → "tea_resumed" (added 2026-07-17; the official app's `TeaRestartBleModel` treats it the same — bare notification, no payload). The firmware fires these between steeps inside one tea recipe — entities can listen via the event bus rather than orchestrating per-steep. **Every event type fired in `_client.py`'s maps must also be in `event.py`'s `NOTIFICATION_EVENT_TYPES`/`ERROR_EVENT_TYPES` + the three translation files** — HA's `EventEntity._trigger_event` raises on unregistered types (bit us live with `tea_soak_time_changed`, which was in `_NOTIFICATION_MAP` but never registered); `tests/test_event_type_registration.py` now pins the invariant.
...
- **A command-table completeness sweep (2026-07-17) turned up 8 unhandled command IDs. An initial pass (androguard ID-sweep + command names only) implemented 2 and guessed at the rest; a follow-up `jadx` decompile of `xbloom_coffee_release.apk`'s actual model classes (`com/chisalsoft/andite/model/ble/*BleModel.java`) corrected two of those guesses and turned 3 more "no concrete use case" items into real, payload-confirmed features.** Lesson reinforced: `androguard`'s ID sweep tells you a command exists; only a `jadx`-level source read of the class `BaseBleModel.Companion.create()` dispatches to (per command id) tells you what the payload means and where it's actually triggered from — don't guess from the command name.
  - **`RD_Pods` (40501) → `pod_detected` notification event, `xid` attribute — payload size fixed 2026-07-17.** `PodsBleModel`: `data.substring(0, 12)` is 12 **hex characters**, i.e. 6 raw bytes, hex-decoded byte-by-byte to ASCII (no reversal) — the initial implementation read 12 raw bytes instead of 6. Fixed in `_client.py` (`payload[:6]`, still filtered through `strict_ascii()` as a defensive layer the official app doesn't bother with).
  - **`RD_EASYMODE_BEGIN` (8111) → `easy_slot_started` notification event, `slot` attribute (`"A"`/`"B"`/`"C"`) — implemented 2026-07-17, payload confirmed via `jadx`.** Fires when an Easy Mode brew is started from the machine's own dial (not from HA). `EasyModeBeginBleModel`: payload is a single LE uint32; `AppBaseActivity.showEasyModeRecipeIfNeed(mode)` bails unless `0 <= mode <= 2` and then indexes a 3-element Easy Slot list with it directly — slot index is confirmed, not a guess. (`RD_EASYMODE_RECIPE_STATE`/11518, originally paired with 8111 as "maybe also slot-related," turned out to be unrelated — see below.)
  - **`RD_CurrentGrinder` (40526) → `grinder.size` parity, same `-30` offset as `RD_GRINDER_SIZE` — implemented 2026-07-17, corrects an earlier wrong guess.** The initial pass guessed this duplicates `RD_GRINDER_SIZE`/`RD_GearReport` and skipped it; `jadx` shows `CurrentGrinderBleModel` parses a real LE uint32 and `GrinderActivity` maps it with the identical `value - 30` the vendored `RD_GRINDER_SIZE` handler uses — genuine parity gap, not a duplicate. It fires in contexts our own brew flow doesn't otherwise trigger (standalone Grinder screen, grinder calibration below).
  - **Grinder gear-position calibration progress/completion tracking — `50038`/`50039`/`40526==85`/`RD_Grinder_Stop`, added 2026-07-17 on top of the cmd `3502` trigger the "Advanced Features" `advanced_settings` service (`calibrate_grinder`) already had since 2026-07-16 — a same-day duplication mistake and its receive-side fix.** The cmd 3502 trigger itself (`CalibrateGrinderActivity`'s confirm button, `CodeModule(3502, "磨豆档位归0", 1000)`) was **not new** — `_client.py`'s `CMD_CALIBRATE_GRINDER`/`client.async_calibrate_grinder()` and `async_set_advanced_settings(calibrate_grinder=True)` already existed. A grep for prior art used `CALIBRATE` (case-sensitive) against the lowercase `calibrate_grinder`, missed it, and a redundant second trigger path (`brewing.async_calibrate_grinder()` + a standalone `button.calibrate_grinder`) got built and shipped before the duplication was caught and reverted the same day — the working, single trigger is `async_set_advanced_settings(calibrate_grinder=True)` only, matching the deliberate "one service, not several always-visible entities for settings nobody adjusts often" design the rest of Advanced Features already follows. What genuinely was missing and got added: `CalibrateStartModel`/`CalibratingModel` (50038/50039) are bare start/in-progress pulses (no payload); completion is `CurrentGrinderBleModel.value == 85` observed while a calibration is in flight. **Hardware-confirmed the same day** (via the since-removed standalone button, before the consolidation — the BLE behavior is identical either way): a real ~120s sweep ran (live grind-size telemetry updated throughout, an ordinary `RD_Grinder_Stop`/`grinding_complete` fired at the end) — but 50038/50039 never arrived at all, and `RD_CurrentGrinder` never reported exactly 85 either, so a naive design that only sets `is_calibrating_grinder` on receiving 50038 would never fire `grinder_calibration_started`/`_progress`/`_complete`, and `sensor.state` would never show anything but idle. Fixed: `async_set_advanced_settings`'s `calibrate_grinder` branch sets `is_calibrating_grinder` and fires `grinder_calibration_started` itself at **send** time instead of waiting for 50038 (which stays wired up as a best-effort safety net only); completion now has two independent paths — the original `raw == 85` check, and (new) `RD_Grinder_Stop` firing `grinder_calibration_complete` whenever a calibration is still in flight when the grinder actually stops, since that's the one signal hardware-confirmed to be reliable. `coordinator._async_update_data` also now surfaces `sensor.state == "calibrating"` (highest priority in the state-derivation chain, since we know for certain — we triggered it) while `client.is_calibrating_grinder()` is true. **50038/50039's names ("Calibrate…") originally led this integration to (wrongly) suspect they were the unresolved *scale*-calibration trigger** — they're grinder-calibration, a different, already-documented feature; the scale-calibration trigger is still unresolved, see the "Advanced Features" bullet. **Also fixed the same day**: `live_grind_speed` (RPM) was coerced from `0` to `None`/"Unknown" via a `value or None` idiom in `coordinator.py` — appropriate for `live_grind_size` (0 is never a real grind size, only "not yet observed") but wrong for speed, where 0 is a real, meaningful "not spinning" reading. `_client.py`'s `RD_Grinder_Stop` handler now explicitly zeroes `grinder.speed` (the firmware may simply stop sending speed telemetry once the grinder stops rather than pushing a final 0, which would otherwise leave the sensor stuck at its last nonzero value), and `coordinator.py` no longer converts a genuine `0` reading to `None` for this field. **Split back out to its own `button.calibrate_grinder` the same day (again)**: a real `xbloom.advanced_settings` call with `calibrate_grinder: true` and a real `config_entry_id` failed with "No XBloom machine matched the service call." Root cause turned out to be unrelated to calibration specifically — a pre-existing bug in `_coordinators_for_call` (see below) broke every targeted call to every service — but the report was reason enough to reconsider bundling a one-shot trigger action into a settings-values service at all: a button doesn't depend on `config_entry_id` resolution, is simpler, and matches how every other one-shot action in this integration (cancel, tare, grind, pour) is exposed. `coordinator.async_calibrate_grinder()` is back as its own method (same send-time flag/event logic, just no longer inside `async_set_advanced_settings`), `async_set_advanced_settings` lost its `calibrate_grinder` parameter, and `llm/calibrate.py` calls the coordinator method directly instead of the service.

- **`_coordinators_for_call`'s `config_entry_id` handling was iterating a string character-by-character since the very first commit — fixed 2026-07-17, hardware-reported via the `advanced_settings` failure above.** HA's `ConfigEntrySelector` has no `multiple` option (confirmed against core's own `helpers/selector.py` — see the device-registry section below), so `call.data.get("config_entry_id")` is always a bare string when provided, never a list. The old code did `entry_ids = call.data.get("config_entry_id") or []` then `for eid in entry_ids:` — for a non-empty string, `for eid in entry_ids` iterates individual *characters* (Python strings are iterable), so no single character could ever match a real config entry id in the coordinator lookup dict, and the call always fell through to "No XBloom machine matched the service call." This silently broke targeting a specific machine on **every** service that goes through this helper (`execute_recipe`, `execute_tea_recipe`, `list_recipes`, `create_recipe`, `edit_recipe`, `delete_recipe`, `write_recipe_to_easy_slot`, `advanced_settings`, `cloud_import_recipe`, `cloud_export_recipe`, `cloud_search_collective_recipes`) — the "no selection = all machines" fallback path was unaffected, which is presumably why it went unnoticed until someone with a specific `config_entry_id` in hand tried it. Fixed to treat `config_entry_id` as the single id it actually is: a direct dict lookup, not a loop. Tests: `tests/test_coordinators_for_call.py`.
- **`binary_sensor.problem`'s water-level derivation trusted the connect-time `RD_MachineInfo` snapshot directly once a serial number was known — fixed 2026-07-17, hardware-reported as "물 수위가 문제있음" (water level shows a problem) after a normal reconnect.** `coordinator._async_update_data` used to do `water_ok = bool(s.water_level_ok) if s.serial_number else not self._water_shortage` — i.e. once MachineInfo had been observed even once, it trusted the raw flag unconditionally. This directly contradicts the flag's own documented unreliability a few lines above in this same file (`_status.water_level_ok` is set only from the one-shot connect-time snapshot and "may still stay False... at idle" on multiple firmwares) — on a unit whose connect-time snapshot happens to read False with a full tank, and that never happens to fire a follow-up `RD_ErrorLackOfWater`/40522 to self-correct (the common case — most sessions have no water error), the "problem" reading was permanent for that whole connection. This bug predates every session in this file's history (present since the initial vendored-wrapper commit) — only now surfaced by a real report. Fixed: `water_ok` is now derived purely from the event-driven `_water_shortage` flag unconditionally, dropping the `serial_number`-gated branch entirely — it already starts optimistic (no shortage) and only flips on an actual `water_shortage`/`water_refilled` notification, which is the behavior the removed branch's own neighboring code comment already described as correct.
- **bleak's own `.mtu_size` property warns on every write until the real MTU is negotiated — cosmetic, suppressed 2026-07-17.** Hardware-reported: 4 identical `UserWarning: Using default MTU value. Call _acquire_mtu() or set _mtu_size first...` log lines across the first few writes after connect. `_client.HABleakConnection.write_command` already falls back to `23` (bleak's own conservative BLE-spec minimum) when the real MTU isn't known yet — functionally correct, just noisy. Wrapped the property access in `warnings.catch_warnings()`/`filterwarnings("ignore", ...)` scoped to just that one message, rather than calling bleak's private `_acquire_mtu()` ourselves.
  - **`8017` (`APP_RECIPE_START_QUIT`/quitRecipeStart) — implemented 2026-07-17 as a standalone `button.dismiss_pod`, then folded into `async_cancel()` the same day.** The initial pass concluded this had "no applicable scenario in our architecture" because our own brew flow sends `8001/8004 → 8002` back-to-back with no confirmation gap. That reasoning doesn't apply here: `jadx` call-site tracing shows `quitRecipeStart()` is sent from `PodsDetailActivity`/`RecipeDetailActivity`'s `showStartDialog()` dismiss handler, **before any BLE brew command is sent** — the machine shows its own local "ready to brew this pod" prompt the instant it reads the NFC tag (the same `RD_Pods`/40501 event now exposed as `pod_detected`), independent of whether the app/HA has armed anything. First shipped as its own `button.dismiss_pod` (`brewing.async_dismiss_pod_prompt()`, bare 8017); removed the same day in favor of folding the same call directly into `coordinator.async_cancel()` — logically it's the same "cancel" action from the user's perspective, just targeting a different machine state, matching the existing `pause`/`resume` toggle-by-state pattern rather than adding a second cancel-shaped button. `_dispatch_event` now tracks `self._pod_prompt_active` (set on `pod_detected`, cleared once a real brew actually starts — `grinding_started`/`brewing_started` — since the app's own arm+execute flow never sends 8017 either); `async_cancel()` sends bare 8017 when that flag is set, the original stop/quit/back-to-home sequence otherwise.
  - **Still deliberately not implemented, not worth re-raising without new decompile evidence:** `RD_EASYMODE_RECIPE_STATE` (11518 — `jadx`-confirmed to be a second, redundant mode-echo compared against the same PRO/EASY codes `RD_EASYMODE_TYPE`/11511 already gives us via `_mode_ack_hex`, used by `MachineJ15Fragment` only to refresh/sync the mode display — not slot- or recipe-progress-related despite the name); `RD_MachineInfo`'s `systemStatus` byte (payload[34], still unresolved — not touched by this sweep); `RD_BYPASS`/`RD_BeforeVibration` (40520/40527 — `jadx`-confirmed genuinely payload-less pulses, `RxBus.get().post()` with no arguments, zero state to expose); `8003`/`8014`/`8007` (scale/pour-screen enter-exit handshakes for manual utility screens this integration doesn't implement, same category as the standalone Grinder/Brewer screens above).
  - Tests: `tests/test_pod_detection_and_weight_parity.py`. **Not hardware-verified** — no real Bluetooth reachable in this dev environment (see Testing below); needs a real pod insert, a real Easy Mode dial-start, and a real grinder calibration run to confirm on actual hardware.
...
- **Outbound writes are split into ≤100-byte chunks (2026-07-17), matching the official app's fastble `setSplitWriteNum(100)`.** The app requests MTU 100 and splits *every* write at 100 bytes, so the firmware demonstrably reassembles a command from multiple BLE writes (framing is header+length; write boundaries mid-packet are fine). We previously sent each packet as one unfragmented write-without-response, relying on the negotiated MTU being large enough — fine on a local adapter (MTU ~517) but a silent-truncation risk for long recipe payloads (8001/8004, Easy Slot 11510) on low-MTU paths like ESPHome BLE proxies. `_client.HABleakConnection.write_command` now chunks at `min(100, mtu_size - 3)` (floor 20) via `_split_write_chunks` — see `tests/test_split_write.py`. Not hardware-verified over a real low-MTU proxy path yet; on a normal adapter packets under 100 bytes behave exactly as before.
...
**`temperature_c` accepts `"RT"`/`"BP"` names alongside a plain positive int** (`schema.py`'s `_coerce_temperature_c`, added 2026-07-17) — decompiled from `TemperatureConstant.java`/`RecipePourViewHolder`: the official app's own pour-temperature slider snaps to fixed literal values at its min/max, **not** protocol-level sentinels — RT (Room Temperature) = `20`, BP (Boiling Point) = `98` (J15). A recipe already using `temperature_c: 98` behaves identically; this only adds the app's own names as a convenience, matching the existing `_coerce_pour_pattern` string-or-int pattern. Resolves an old open question in `docs/en/brewing-notes.md` ("BP temperature observation... likely a computed near-boiling value, not a fixed sentinel") — it's a fixed constant after all, not computed; the observed byte 99 vs. the constant's 98 is a ~1° rounding/encoding artifact, not evidence of dynamic computation.
