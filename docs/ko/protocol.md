...
- 디스케일링과 저울 캘리브레이션에는 **BLE 명령 계열이 없습니다** (jadx
  ...
  있습니다.
- **저울 캘리브레이션 실행 중 코드는 brewing 코드를 재사용합니다** (하드웨어
  2026-08-24, 처음으로 끝까지 완주한 실행): `0x39` 확인 → `0x3A` NO LOAD →
  `0x3B` 측정 → `0x3F` "+100 g" → `0x3B` "+500 g"/최종 측정 → `0x25` 완료 →
  `0x01` 홈. `0x3B`는 brewing 코드이기도 하므로 문맥으로만 구분됩니다 —
  `client._scale_calibrating`이 확인 화면에서 래치되고, 실행 코드 집합을
  벗어난 첫 코드에서 해제됩니다. 이게 없으면 캘리브레이션 대부분 구간에서
  상태 센서가 `brewing`으로 표시됩니다. 디스케일링 실행 중 코드는 아직
  미캡처입니다(해당 세션은 확인 화면에서 취소됨).
