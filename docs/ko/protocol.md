...
| 9003 | `RD_GRINDER_BEGIN` | 2× LE u32 `(분쇄도, rpm)` | Active, 맥락 의존 | **노브로 시작한** 그라인딩에서는 발화함 (하드웨어 2026-07-20: 40506보다 ~1초 앞서 설정 스냅샷 payload와 함께 일관되게 발화) — 앱/레시피가 시작한 그라인딩에서는 여전히 관측된 적 없음; 모든 맥락에서 발화하는 begin 신호는 40506(아래) |
...
| 9005 | `RD_BREWER_BEGIN` | 노브 시작 시 4× LE u32 `(용량, 온도℃, 패턴, 온도℃ 반복)` | Active, 조기 발생 | 커밋 직후 즉시 발생 — 실제 추출 시작보다 훨씬 이름. 노브로 시작한 추출에서는 페이지 설정 스냅샷을 payload로 실어 발화 (하드웨어 2026-07-20: 직전에 4510/8016 진입 푸시로 설정한 값을 그대로 echo) |
...
| 40511 | `RD_Brewer_Stop` | — | Active, 자연 완료 전용 | 추출 종료 — **4507 정지 후에는 오지 않음** (하드웨어 2026-07-20: 4507 ACK, 물 정지, 머신 추출 페이지 복귀까지 됐는데 40511은 끝내 안 옴; 클라이언트는 대신 페이지 보고에서 실행 플래그를 클리어) |
...
| 40521 | `RD_MachineInfo` | 고정 오프셋 구조체 | Active | 연결 시점 스냅샷: 시리얼, 펌웨어, 모드, 급수 정상 비트, 굵기/전압; 일부 펌웨어에서는 아예 오지 않을 수 있음 — coordinator가 재시도 후 폴백 |
...
- 디스케일링과 저울 캘리브레이션에는 **BLE 명령 계열이 없습니다** (jadx
  ...
  있습니다.
- **저울 캘리브레이션 실행 중 코드는 brewing 코드를 재사용하고, 실행마다
  단계 코드가 다릅니다** (하드웨어 2026-08-24, 완주 캡처 2회):
  `0x39` 확인 → `0x3A` NO LOAD → `0x3B` 측정 → `0x3F` "+100 g" → `0x3B` →
  (`0x3C`, 2회차에만) → `0x3B` → `0x25` 완료 → `0x01` 홈.
  `0x3B`는 brewing 코드이기도 하므로 문맥으로만 구분됩니다:
  `client._scale_calibrating`이 확인 화면에서 래치되고, `0x25` 또는 캘리브레이션
  외의 알려진 의미를 가진 첫 코드(페이지 코드, 또는 모호한 `0x3B` 이외의 상태
  코드)에서 해제됩니다. 미매핑 코드는 캘리브레이션의 추가 단계로 간주합니다 —
  실행 코드를 열거하는 방식은 2회차의 `0x3C`에서 래치가 중간에 풀렸습니다.
  이게 없으면 캘리브레이션 대부분 구간에서 상태 센서가 `brewing`으로
  표시됩니다. 디스케일링 실행 중 코드는 아직 미캡처입니다(해당 세션은 확인
  화면에서 취소됨).
- **캘리브레이션 진행 중에 연결하면** (사용자 보고 2026-08-24) 래치를 거는
  확인 화면을 놓치므로 실행 구간의 `0x3B`가 다시 `brewing`으로 보고됐습니다.
  두 갈래로 수정: 캘리브레이션 실행 중에만 관측된 `0x3C`/`0x3F`도 래치를
  걸고, `0x3B`는 **추출 문맥이 있을 때만** brewing으로 보고합니다 — 마지막
  페이지/ready/유지보수 코드 이후의 `0x1E`/`0x1F`/`0x22`/`0x10`/`0x23`, 또는
  cmd 태그 경로가 이미 세운 그라인더/브루어 실행 플래그. 둘 다 없으면 실제로
  일어나지 않는 추출을 알리는 대신 라벨을 붙이지 않습니다(cmd 태그 상태로
  폴백).
- **설정 페이지 `0x17`/`0x18`/`0x19`** (사용자 로그 2026-08-24): 사용자가
  머신 자체의 단위 설정 페이지 — `8005`/`8010`/`4508` SET 명령이 여는 그
  화면 — 를 넘길 때 보고되며, 마지막에 새 값이 `8015`로 되돌아오고 `0x01`
  홈으로 복귀합니다. 어느 코드가 어느 페이지인지는 미확정이라 셋 다 하나의
  `settings` 화면 라벨로 매핑합니다.
- **시작 전환 드롭 구간** (하드웨어 2026-07-20): 노브 시작의 begin 보고
  (9003/9005)와 run-begin(40506) 사이에 도착한 명령은 조용히 드롭될 수
  있습니다 — 9003 후 ~1.9초에 보낸 3505는 무시됐고(ACK 없음, 그라인더
  계속 동작) 40506 이후의 동일 전송은 즉시 정지시킴. 결정적이지 않음
  (다른 시도에서 9003 후 0.2초 전송은 동작). 정지는 실행 플래그가 실제로
  꺼질 때까지 미확정으로 취급하고 1회 재전송할 것
  (`coordinator._async_verify_component_stop`).
- **연속 전송 시 두 번째 명령 드롭**: 4510과 8016을 1ms 간격으로 보내면
  4510만 ACK되고 8016은 무시됨(머신은 기억된 패턴 유지). 공식 앱은
  `sendMessage` 큐가 ACK 게이트(측정 지연 ~370-380ms)라 이 문제를 피함;
  한 흐름에서 연속 전송 사이에 ≥0.5초(또는 ACK 게이트)를 둘 것.
  `8005`/`8010`/`4508`도 같은 형태로 당했습니다(사용자 보고 2026-08-24):
  셋이 5ms 안에 모두 나갔고 `8005`만 echo됐으며 급수 방식 변경은 머신에
...
- **설정 SET은 머신을 해당 설정 페이지에 남겨둡니다**. 사용자가 직접 확인해
  ...
  **8022를 ACK 직후에 보내면 안 됩니다** — 페이지는 SET이 ACK된 *다음에*
  열립니다. 맥에서 직접 측정(2026-08-24): `4508`은 전송 0.38초 후 ACK,
  TAP/TANK 페이지(`0x19`)는 그로부터 0.46초 뒤 등장. ACK 직후 보낸 `8022`는
  자신은 ACK됐지만 아무 효과가 없었고(남은 16초 동안 `0x19` 유지), 2.5초
  뒤에 보낸 동일 명령은 0.5초 내에 `0x01` 홈으로 복귀시켰습니다. 2.5초는
  앱의 `MachineJ15Fragment.backToHomeIn` 기본값(2500ms)이기도 합니다.
  같은 캡처의 부수 관찰: 탱크가 비어 있으면 `4508` 약 90ms 뒤에 `40522`
  (`RD_ErrorLackOfWater`) 값 0이 따라옵니다 — TAP 사용 시 정상이며, 앱은
  `waterFeed != 0`인 동안 이 경고를 아예 표시하지 않습니다.
