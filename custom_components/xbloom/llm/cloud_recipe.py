...
class XBloomSearchMyRecipesTool(XBloomBaseTool):
    """Browse the logged-in account's own Product or Shared recipe tab."""

    name = "search_xbloom_my_recipes"
    description = (
        "Browse the logged-in XBloom cloud account's own Product tab "
        "(recipes bundled with a purchased pod) or Shared tab (recipes "
        "another user pushed to this account) — account-private, unlike "
        "search_xbloom_collective_recipes, and requires a configured "
        "cloud account. Each result is an independent copy, not a "
        "live-synced list, and is NOT directly runnable — pass its "
        "cloud_table_id to import_xbloom_my_recipe first to save it "
        "locally, then run it via execute_xbloom_recipe."
    )
    parameters = vol.Schema(
        {
            vol.Required(
                "recipe_type", description="Which account tab to browse."
            ): vol.In(["product", "shared"]),
            vol.Optional(
                "keyword", description="Free-text search across recipe names."
            ): str,
        }
    ...
    async def async_call(
        ...
        args = tool_input.tool_args
        result = await self.coordinator.async_search_my_recipes(
            args["recipe_type"], keyword=args.get("keyword")
        )
        if not result.get("success"):
            return _cloud_failure(result, f"{args['recipe_type']} recipe search")
        return {
            "success": True,
            "recipes": result["recipes"],
            "instruction": (
                "Read out the recipe names (and cloud_table_id if the "
                "user wants to import one via import_xbloom_my_recipe). "
                "These are not runnable until imported."
            ),
        }


class XBloomImportMyRecipeTool(XBloomBaseTool):
    """Import a recipe from the account's Product or Shared tab by table id."""

    name = "import_xbloom_my_recipe"
    description = (
        "Import one recipe from the logged-in account's Product or "
        "Shared tab (see search_xbloom_my_recipes) into the local recipe "
        "store by its cloud table id, so it shows up in "
        "list_xbloom_recipes / execute_xbloom_recipe. Requires a "
        "configured cloud account — unlike import_xbloom_cloud_recipe, "
        "these tabs have no public share link."
    )
    parameters = vol.Schema(
        {
            vol.Required(
                "recipe_type",
                description="Which account tab the table id came from.",
            ): vol.In(["product", "shared"]),
            vol.Required(
                "table_id",
                description="The recipe's cloud table id, from search_xbloom_my_recipes.",
            ): int,
        }
    )
    ...
    async def async_call(
        ...
        args = tool_input.tool_args
        result = await self.coordinator.async_import_my_cloud_recipe(
            args["recipe_type"], args["table_id"]
        )
        if not result.get("success"):
            return _cloud_failure(result, "import")
        return {
