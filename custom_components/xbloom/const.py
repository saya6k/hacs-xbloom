...
XBLOOM_LLM_PROMPT = (
    ...
    "Use pour_xbloom for a manual pour with a specific temperature (°C) "
    "and volume (ml), and grind_xbloom for a manual grind with a specific "
    "grind size and RPM — grind_xbloom and pour_xbloom are separate "
    "manual actions (grind does not pour, pour does not grind); use "
    "execute_xbloom_recipe instead to do both in one recipe-driven brew. "
    "Both are two-phase: the first call ARMS the machine (it opens the "
    "matching page showing the settings without starting), then you ask "
    "the user to confirm and call the same tool again with "
    "confirmed=true to start; if the user declines, call cancel_xbloom. "
    "Use cancel_xbloom to back out of an armed operation or stop a "
    "running grind/pour/brew. Use tare_xbloom_scale to zero the scale. Use "
