"""Tests for XBloomCoordinator.async_pour sending bare BREWER_START (4506)
...
package split (main branch), so there's no separable OperationsMixin to
...
_async_pour = XBloomCoordinator.async_pour
...
def test_async_pour_sends_only_bare_brewer_start():
    fake_self = _FakeSelf()
    asyncio.run(_async_pour(fake_self))

    # No 8007 (or any other bare command) sent before the pour.
    assert fake_self.client.sent_commands == []
    assert len(fake_self.client.brewer.start_calls) == 1


def test_async_pour_passes_current_slider_values():
    fake_self = _FakeSelf()
    fake_self.volume = 250.0
    fake_self.temperature = 88.0
    asyncio.run(_async_pour(fake_self))

    call = fake_self.client.brewer.start_calls[0]
