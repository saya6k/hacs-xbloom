"""Tests for XBloomCoordinator.async_write_easy_slot refusing tea recipes.
...
package split (main branch), so there's no separable RecipesMixin to
import here (same pattern as tests/test_wake_retry.py). The check runs
before any connection/BLE work, so only `.recipes` / `.selected_recipe`
/ `._check_connected` are needed.
"""
...
_async_write_easy_slot = XBloomCoordinator.async_write_easy_slot
